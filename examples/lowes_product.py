# -- coding: utf-8 -*-
"""
Lowes 产品页爬取 - 基于 iv8 的 Akamai 绕过
...
# ── 配置 ──────────────────────────────────────────────
PRODUCT_URL = "https://www.lowes.com/pd/Cascade-Cascade-33840-75-oz-Cascade-Complete-Gel/5001374747"
...
def inject_cookies_to_iv8(ctx, cookies, domain=".lowes.com"):
...
JS_REINFORCE = """(function() {
    ...
    // history.length 兜底（Akamai 用此值判断是不是新开窗口）
    try {
...
def simulate_user_input(ctx):
    """模拟鼠标移动事件，让 sensor_data 包含真实输入轨迹（isTrusted=true）"""
    ctx.eval("""
        (function() {
            try {
                ...
                try { __iv8__.input.dispatchMouseEvent({type:'mousedown', target:doc, clientX:last[0], clientY:last[1], button:0, buttons:1}); } catch(e) {}
                try { __iv8__.input.dispatchMouseEvent({type:'mouseup',   target:doc, clientX:last[0], clientY:last[1], button:0, buttons:0}); } catch(e) {}
                try { __iv8__.input.dispatchMouseEvent({type:'click',     target:doc, clientX:last[0], clientY:last[1], button:0, buttons:0}); } catch(e) {}
            } catch(e) {}
        })();
    """)
...
def find_sensor_data_in_netlog(entries):
    """
    Step 5: 在 netLog.entries 中找到 Akamai sensor POST
...
def run_akamai_bypass(session, html, page_url, scripts, resources):
    """
    ...
    with iv8.JSContext(environment=environment, config={"time": {"mode": "logical"}}) as ctx:
        ...
        # 注入初始 cookie（关键！Akamai JS 需要读取初始 _abck 来生成正确的 sensor_data）
        ...
        # Step 5: 从 netLog.entries 提取 sensor_data
        entries = ctx.eval("window.__iv8__.netLog.entries", to_py=True) or []
        print(f"    [iv8] netLog 捕获到 {len(entries)} 个请求")

        # 保存 netLog 用于调试
        debug_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "netlog_debug.json")
        try:
...
def main():
    print("=" * 60)
    print("Lowes 产品页爬取 - iv8 Akamai 绕过")
    print("=" * 60)
    ...
    #
    # # 先访问主页预热
    # print("    预热: GET https://www.lowes.com/")
    # r0 = session.get(PRODUCT_URL, headers=HEADERS, proxy=PROXY, timeout=30)
    # print(f"    预热状态: {r0.status_code}, cookies: {list(r0.cookies.keys())}")
    #
    # if is_akamai_challenge(r0.text):
    #     print("    [错误] 主页就是挑战页 - IP 可能被封")
    #     return
    #
    # time.sleep(2)  # 模拟人类行为

    # 访问目标产品页
    product_headers = {**HEADERS, "referer": BASE_URL, "sec-fetch-site": "same-origin"}
    r1 = session.get(PRODUCT_URL, headers=product_headers, proxy=PROXY, timeout=30)
    ...
    if r1.status_code == 200 and not is_akamai_challenge(r1.text):
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "lowes_product.html")
    ...
    if r1.status_code != 200:
        ...
        # 保存错误响应用于排查
        err_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"lowes_error_{r1.status_code}.html")
    ...
    sensor_url, sensor_body = run_akamai_bypass(session, html, PRODUCT_URL, scripts, resources)
    ...
    # 检查 _abck 状态
    abck = session.cookies.get("_abck", domain=".lowes.com") or session.cookies.get("_abck")
    if abck:
    ...
    # 再次请求产品页
    print("\n[验证] 再次请求产品页...")
    r2 = session.get(PRODUCT_URL, headers=product_headers, proxy=PROXY, timeout=30)
    ...
    if r2.status_code == 200 and not is_akamai_challenge(r2.text):
        ...
        challenge_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "lowes_challenge.html")
