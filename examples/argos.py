"""
Argos.co.uk 页面访问 - 基于 httpcloak + iv8 的 Akamai 绕过

Argos 使用 Akamai Bot Manager v2/v3，流程与 lowes_product.py 相同：
...
TARGET_URL = "https://www.argos.co.uk/features/love-the-game?tag=ar:homepage:newhp:section1:love-the-game:block1"
...
def inject_cookies_to_iv8(ctx, session, domain=".argos.co.uk"):
...
def run_akamai_bypass(session, html, page_url, resources):
    ...
    with iv8.JSContext(environment=environment, config={"time": {"mode": "logical"}, "timezone": "Europe/London"}) as ctx:
        snapshot = {
            "baseURL": page_url,
            "html": unescape(html),
            "resources": resources,
        ...
        debug_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "argos_netlog_debug.json")
        try:
...
def main():
    print("=" * 60)
    print("Argos.co.uk 访问 - httpcloak + iv8 Akamai 绕过")
    print("=" * 60)
    ...
    if r1.status_code == 200 and not is_akamai_challenge(r1.text):
        ...
        out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "argos.html")
    ...
    if not scripts:
        ...
        with open("argos_challenge_debug.html", "w", encoding="utf-8") as f:
            f.write(r1.text)
        return
    ...
    if not sensor_url or not sensor_body:
        print("    [错误] 未捕获到 sensor_data，查看 argos_netlog_debug.json")
        return
    ...
    print("\n[验证] 再次 GET 目标页...")
    r2 = session.get(TARGET_URL, headers={**HEADERS, "referer": "https://www.argos.co.uk/"})
    ...
    if r2.status_code == 200 and not is_akamai_challenge(r2.text):
        ...
        with open("argos_challenge2.html", "w", encoding="utf-8") as f:
            f.write(r2.text)
        print("    [失败] 仍有挑战，已保存到 argos_challenge2.html")
